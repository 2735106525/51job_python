...
class jobs:
    ...
    def url(self):     #构造url
        for i in range(1, 2):
            ...
            url_end = ".html?"
            self.url = url_pre + str(i) + url_end


    def headers(self):   #构造请求头
    ...
    def parsing(self):     #分析提取数据
        web = requests.get(url=self.url, headers=self.headers)
        web.encoding = "gbk"
        ...
        for i in range(len(deep_url)):
            web_test = requests.get(deep_url[i], headers=self.headers)
